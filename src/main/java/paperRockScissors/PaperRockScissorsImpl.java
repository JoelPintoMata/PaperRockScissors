...
//        decrease the number of plays left to play
    ...
    protected Optional<Integer> getHandResult(List<Play> plays) {
        List<Play> play1 = plays.stream().filter(x -> x.getShape().equals(ShapesEnum.PAPER)).collect(Collectors.toList());
        List<Play> play2 = plays.stream().filter(x -> x.getShape().equals(ShapesEnum.ROCK)).collect(Collectors.toList());
        List<Play> play3 = plays.stream().filter(x -> x.getShape().equals(ShapesEnum.SCISSORS)).collect(Collectors.toList());
